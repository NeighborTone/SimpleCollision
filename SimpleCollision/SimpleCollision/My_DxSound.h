...
public:
	...
	~Sound() {};														//デストラクタ
	void SetSE(const TCHAR *FileName);									//第一引数にSEをのパスを指定しロードする。複数ロードでき、配列の後ろに格納される(0~)
	void SetBGM(const TCHAR *FileName);									//BGMのパスを指定してロードし、再生フラグを立てる
	...
	void MultiPlayBGM_Loop();											//複数のBGMを同時ループ再生する
	void ChangeBGMVolume(int BGM_ID,int gain);							//指定したBGMの音量を変更する
	void PlaySE(int SE_ID,int gain = 100);								//第一引数に格納されたSEのIDを指定し再生、引数に音量を指定できる(0~100)デフォルトは100%
